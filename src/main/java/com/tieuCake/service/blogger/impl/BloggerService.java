...
public class BloggerService implements IBloggerService {
	...
	public Blogger getBlogger() throws IOException, InvalidValueException {
		if (null == blogger) {
			...
			blogger = new Blogger.Builder(new NetHttpTransport(),
	...
	public String getBloggerName() throws Exception {
		try {
			...
			for (String blogId : BloggerUtil.BLOGGERIDS) {
				...
				// This is the request action that you can configure before
				// sending the request.
				Blogger.Blogs.Get blogGetAction = blogger.blogs().get(blogId);
				// Restrict the result content to just the data we need.
				blogGetAction
						.setFields("description,name,posts/totalItems,updated");
				// This step sends the request to the server.
				Blog blog = blogGetAction.execute();
				// Now we can navigate the response.
				System.out.println("Name: " + blog.getName());
				result = result + "Name: " + blog.getName() + "\t Post Count:"
						+ blog.getPosts().getTotalItems();
	...
	public Integer synchPublicPostDataFromBlogger() throws Exception {
		try {
			logger.info("Get the google credential");
			Blogger blogger = getBlogger();
			List<Post> results = new ArrayList<Post>();
			for (String blogId : BloggerUtil.BLOGGERIDS) {
				results = new ArrayList<Post>();
				logger.info("Set the related info to get the Posts from Blogger");
				// The request action.
				Blogger.Blogs.Get blogGetAction = blogger.blogs().get(blogId);
				Blog blog = blogGetAction.execute();
				Blogger.Posts.List postsListAction = blogger.posts().list(
						blogId);
				// Restrict the result content to just the data we need.
				...
				int i = 0;
				logger.info("Set the Posts into the result list");
				// get the time loop
				int time = (blog.getPosts().getTotalItems() / 500.0) <= 0 ? 1
						: (int) (Math
								.ceil(blog.getPosts().getTotalItems() / 500.0));
				while (i < time) {
					PostList posts = postsListAction.execute();
					List<Post> tmpPost = posts
							.getItems()
							.stream()
							.filter(p -> (p.getId() != null && !p.getId()
									.isEmpty()))
							.filter(p -> (!p.getContent().contains(
									"Xem tiếp &gt;&gt;&gt;") && !p.getContent()
									.contains("Còn tiếp &gt;&gt;&gt;")))
				...
				for (Post post : results) {
					PostEntity postEntity = new PostEntity(post.getId(),
							post.getTitle(), post.getAuthor().getDisplayName(),
							post.getContent(), post.getPublished(),
							post.getUrl(), PostStatusEnum.PUBLIC.toString());
